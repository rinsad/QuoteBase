...
export async function saveOpenAIIntegration(formData: FormData) {
  ...
  const parsed = openAISettingsSchema.safeParse({
    ...
    model: getString(formData, "model") || "gpt-5.4-mini",
    api_key: getString(formData, "api_key"),
  });
  ...
  revalidatePath("/admin/integrations/openai");
  revalidatePath("/dashboard");
  redirect("/admin/integrations/openai?saved=1");
}
